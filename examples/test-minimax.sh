...
if [ -z "${MINIMAX_API_KEY:-}" ]; then
    ...
    echo "Get an API key from https://api.minimax.chat"
    exit 1
fi

export CMDIFY_PROVIDER_NAME="minimax"
export CMDIFY_MODEL_NAME="${CMDIFY_MODEL_NAME:-MiniMax-M2.7}"
...
echo "--- Test 3: Custom base URL (if MINIMAX_BASE_URL is set) ---"
if [ -n "${MINIMAX_BASE_URL:-}" ]; then
    echo "Custom base URL: $MINIMAX_BASE_URL"
    "$CMDIFY_BIN" "show the current git branch"
else
    echo "(skipped - MINIMAX_BASE_URL not set)"
fi
echo ""

echo "=== Minimax Tests Complete ==="

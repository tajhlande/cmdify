...
export CMDIFY_PROVIDER_NAME="mistral"
export CMDIFY_MODEL_NAME="${CMDIFY_MODEL_NAME:-ministral-14b-2512}"

echo "=== Mistral Integration Test ==="
...
echo "--- Test 3: Custom base URL (if MISTRAL_BASE_URL is set) ---"
if [ -n "${MISTRAL_BASE_URL:-}" ]; then
    echo "Custom base URL: $MISTRAL_BASE_URL"
    "$CMDIFY_BIN" "show the current git branch"
else
    echo "(skipped - MISTRAL_BASE_URL not set)"
fi
echo ""

echo "=== Mistral Tests Complete ==="

...
export CMDIFY_PROVIDER_NAME="openrouter"
export CMDIFY_MODEL_NAME="${CMDIFY_MODEL_NAME:-google/gemma-4-26b-a4b-it:free}"
...
echo "--- Test 3: Custom base URL (if OPENROUTER_BASE_URL is set) ---"
if [ -n "${OPENROUTER_BASE_URL:-}" ]; then
    echo "Custom base URL: $OPENROUTER_BASE_URL"
    "$CMDIFY_BIN" "show the current git branch"
else
    echo "(skipped - OPENROUTER_BASE_URL not set)"
fi
echo ""

echo "=== OpenRouter Tests Complete ==="
